...
describe( 'S4 - 00 - When Starting a Session - Returning named user < 30 min, non Expert', function() {  
        ...
        lambda.handler( {
           "session": {
              ...
              "attributes": {
                 ...
                 "lastSession": ( new Date( new Date() - ( 15 * 60 * 1000 ) ) ).toUTCString()
              },
              "user": {
                 "userId": "testUser"
              },
    ...
    describe( 'Attributes are completely defined', function() {
        it( 'sessionCount is 1', function() {
            expect( speechResponse.sessionAttributes.sessionCount ).not.to.be.undefined
            expect( speechResponse.sessionAttributes.sessionCount ).to.equal( 1 )
        } )
        ...
        it( 'name is Julie', function() {
            expect( speechResponse.sessionAttributes.name ).not.to.be.undefined
            expect( speechResponse.sessionAttributes.name ).to.equal( 'Julie' )
        } )
    ...
    describe( 'Speech response is as expected', function() {
        ...
        it( 'output ssml contains intro', function() {
            ...
            expect( speechResponse.response.outputSpeech.ssml.includes( "Julie, Welcome back.  What can I do for you?" ) ).to.be.true
        } )
    } )
    ...
    describe( 'Speech reprompt response is as expected', function() {
        ...
        it( 'reprompt ssml contains intro', function() {
            ...
            expect( speechResponse.response.reprompt.outputSpeech.ssml.includes( "  You can ask for the weather, how many times you've visited, when you first visited, when you last visited, and you can change your name.  What would you like to do?" ) ).to.be.true
        } )
    } )
} )
